...
cp ffhb_cal/conf.py ffhb_cal/conf.backup_repo.py || exit
...
sudo chown jimmydore:www-data ../.git/hooks/pre-commit || exit #TODO : Recup LES noms des env virtuels, des user unix dans la config 
...
../ffhb_venv/bin/python manage.py makemigrations ffhb_cal_app || ../myvenv/bin/python manage.py makemigrations ffhb_cal_app || exit
../ffhb_venv/bin/python manage.py migrate || ../myvenv/bin/python manage.py migrate || exit
